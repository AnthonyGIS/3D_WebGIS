    ...
    var LayerCollection = Backbone.Collection.extend({
        ...
        filterByType : function(result,mergeRes){
    		...
    		for(i = 0,j = arr.length;i < j;i++){
    			var item = arr[i];
    			if(mergeRes[item.resTitle + '/rest']){
    				continue;
    			}
    			var url,promise;
    			if(item.type == 'MAP'){
        ...
        setModels : function(colc,isPublic){
        	var models = [];
			for(var m = 0,n = colc.length;m < n;m++){
				var res = colc[m];
				var len = res.length;
				var childModels = [];
				var model;
        ...
        fetch : function(url,parent,isPublic){
            ...
            Cesium.when(Cesium.loadJson(url)).then(function(result){
                ...
                Cesium.when.all(totalPromise).then(function(colc){
                    me.setModels(colc,isPublic);
