...
/**
 *
 * @param container_name  eg: '#webgl_output'
 * @constructor
 */
var HelloWorldThree = function (container_name) {

    if (container_name === undefined) console.warn('no container name.');
    ...
    // 这些内部变量，都是在new一个变量的时候就被初始化执行了，特别是如var obj_xxx = xxxxx；就是在这个new的语句。
    // 此时初始化后，obj_xxx 不是挂载了windows下，而是类内部的局部变量，不能被外部访问到。


    var scene,camera,webgl_render, controls;
    var stats = initStats();
    var container = $(container_name)[0]; // document.getElementById( 'container' );
    ...
    function onWindowResize() {
        camera.aspect = container.offsetWidth / container.offsetHeight; //container.offsetWidth / container.offsetHeight
    ...
    window.addEventListener('dblclick',onWindowResize, false);
...
$(function () {
    ...
    let hello_world_obj = new HelloWorldThree('#webgl_output');
    hello_world_obj.init();
    hello_world_obj.animate();
