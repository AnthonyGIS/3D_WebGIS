...
// 动态原型方式
function
CesiumCoordinateDisplay(viewer) {
    ...
    /* // usage
      ...
      // modify the event type,to accelerate the js run
      Create by WENG LIU; V1.0.1 2019.1.13 1604

      // update lib to recognize of Batched3DModel3DTileContent
      Create by WENG LIU; V1.0.2 2020.6.8
      */
    ...
    if (typeof CesiumCoordinateDisplay._initialized === "undefined") {

        CesiumCoordinateDisplay.prototype.InitialStatusPanel = function () {
        ...
        CesiumCoordinateDisplay.prototype.SetMouseMoveEvent = function () {
            ...
            handler.setInputAction(function (event) {
                ...
                mouse_click_pos = event.position; // 如果是mouse move的话，是event.endPosition;
                ray = viewer.scene.camera.getPickRay(mouse_click_pos);
                ...
                var feature = viewer.scene.pick(mouse_click_pos);
                if (feature === undefined) {
                    lng = Cesium.Math.toDegrees(cartographic1.longitude);
                    lat = Cesium.Math.toDegrees(cartographic1.latitude);
                }
                if (feature instanceof Cesium.Cesium3DTileFeature || (feature !== undefined &&  feature["content"] instanceof Cesium.Batched3DModel3DTileContent) ) {
                    var cartesian = viewer.scene.pickPosition(mouse_click_pos);
                ...
                if (viewer.scene.mode === Cesium.SceneMode.MORPHING) {
                    var pickedObject = scene.pick(mouse_click_pos);
                    if (viewer.scene.pickPositionSupported && Cesium.defined(pickedObject) && pickedObject.node) {
                        cartesian = viewer.scene.pickPosition(mouse_click_pos);
                        if (Cesium.defined(cartesian)) {
                ...
                this.camera_show.innerHTML = (viewer.camera.positionCartographic.height / 1000).toFixed(3); // Cammera Height
                this.status_panel_disp.style.width = (500 + 130 * status_length_plus).toString() + 'px';


            }, Cesium.ScreenSpaceEventType.LEFT_CLICK);//MOUSE_MOVE(影响性能),LEFT_CLICK


        };
        CesiumCoordinateDisplay._initialized = true;
    }

    if (typeof this.alertInfo !== "function") {
        console.log("这段代码只执行一次");
        CesiumCoordinateDisplay.prototype.AddStatusPanelHTML = function () {
