...
//   另一个对象的该属性也会修改。因为来自原型对象的引用属性是所有实例共享的。
var obj = { // 父类的原型对象属性
    0:'a',
    1:'b',
    arr:[1]
...
// 1 构造继承
// 有时候也叫做伪造对象或经典继承，即在子类型构造函数的内部调用超类型构造函数。
...
function Sub2(){
    Super2.call(this);   // 组合继承核心,构造继承，继承属性
    // ...
}
Sub2.prototype = new Super2(); // 组合继承核心,构造继承，继承方法【函数又实例一次，函数内部变量又重复实例一次】
...
// 个参数：子类型构造函数和超类型构造函数。在函数内部，第一步是创建超类型原型的一个副本。
...
Super3.prototype.c = function(){console.log(1111)};

function Sub3(a,b){
    this.a = a;
    Super3.call(this,b);  // 子类构造集成父类
}

//对父类的prototype进行一次寄生，即包装成一个空对象的prototype，
var f = new Function();
f.prototype = Super3.prototype; // 把这个对象实例化出来
Sub3.prototype = new f();// 作为子类的prototype



var sub3_obj = new Sub3(1,2);
