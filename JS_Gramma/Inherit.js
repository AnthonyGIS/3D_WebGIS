...
// 2) 同时new两个对象时改变一个对象的原型中的引用类型的属性时，另一个实例的该属性也会修改。
...
// 2 构造继承
// ----------------------------------------------------------------------------------------
// 有时候也叫做伪造对象或经典继承，即在【子类型构造函数的内部调用超类（父类）型构造函数】。
...
function Foo1(a,b){
    ...
    // 构造继承向父类传参
    // 但是通过这种方式实例化出来的实例，会将父类方法多次存储，影响性能
    Super1.call(this,b);
}
...
// 3 组合继承 （JS中最常用的继承模式）
// ----------------------------------------------------------------------------------------
// combination inheritance， 有时候也叫做伪经典继承。
...
function Sub2(){
    // 组合继承核心----构造继承，构造函数内继承属性
    Super2.call(this);
    // ...
}
// 组合继承核心----原型继承，继承方法【函数又实例一次，父类构造函数中的属性又重复实例一次】
...
// 其背后的基本思路是：不必为了指定子类型的原型，而调用超类型的构造函数。我们所需要的，无非就是超类型原型的一个副本而已。
// 本质上，就是使用（寄生式继承）来继承超类型的原型，然后再将结果指定给子类型的原型。

// 基本模式如下所示:
// 参数：子类型构造函数和超类型构造函数。
function inheritPrototype(subType, superType){
    // 第一步，创建对象。在函数内部，创建超类型原型的一个副本。
    var obj = object(superType.prototype);
    // 第二步，增强对象。为创建的副本添加constructor 属性，从而弥补因重写原型，而失去的默认的constructor属性。
    obj.constructor = subType;
    // 最后一步，指定对象。将新创建的对象（即副本）赋值给子类型的原型。
    // 这样，我们就可以用调用inheritPrototype()函数的语句，去替换前面例子中为子类型原型赋值的语句了。
...
var f = new Function();
// 把这个对象实例化出来
f.prototype = Super4.prototype;
// 作为子类的prototype
Sub4.prototype = new f();
